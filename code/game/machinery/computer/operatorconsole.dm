/obj/machinery/computer/vox_console
	name = "Communications console with vox announcement capabilities"
	desc = "Used by the operator to perform communicaations tasks and vox announcements."
...
/obj/machinery/computer/vox_console/New()
	..()
	crew_announcement.newscast = 1

/obj/machinery/computer/vox_console/process()
	if(..())
		if(state != STATE_STATUSDISPLAY)
			src.updateDialog()


/obj/machinery/computer/vox_console/Topic(href, href_list)
	if(..())
		return 1
	if (src.z > 1)
...
/obj/machinery/computer/vox_console/emag_act(var/remaining_charges, var/mob/user)
	if(!emagged)
		src.emagged = 1
		user << "You scramble the communication routing circuits!"
		return 1

/obj/machinery/computer/vox_console/attack_ai(var/mob/user as mob)
	return src.attack_hand(user)

/obj/machinery/computer/vox_console/attack_hand(var/mob/user as mob)
	if(..())
		return
	if (src.z > 6)
		...
		if(STATE_DEFAULT)
			if (src.authenticated)
				...
				if (src.authenticated==2)
					...
					dat += "<BR>\[ <A HREF='?src=\ref[src];operation=vox_help'>View VOX Help</A> \]"
					if(src.emagged == 0)
...
/obj/machinery/computer/vox_console/proc/interact_ai(var/mob/living/silicon/ai/user as mob)
	var/dat = ""
	switch(src.aistate)
		if(STATE_DEFAULT)
...
/obj/machinery/computer/vox_console/proc/post_status(var/command, var/data1, var/data2)
